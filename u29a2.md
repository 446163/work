...
In addition to this you will need to give each of the following tasks a certain amount of time so that they can actually be completed on time in time for the customer. If you do not plan for this then then you will in most not actually make it to the customer in time and this will lead to a failed product. In the table below I will go through some examples of times that each stage could take:

Stage       | Time      | Description
------------|-----------|------------
Design 		| 2 days 	| The interactions that the software would have with the system is first thought of and written down 
Planning 	| 1 week 	| The plan is written up so that is can be followed, this also includes the plan if the installation does not go well, this also ensures that the software should work on the system
Delivery 	| 2-3 weeks | The software is installed on all the machines that need it
Testing 	| 2-3 weeks | The software is tested to ensure that is all works as the customer needs it

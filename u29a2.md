...
In this section I will be explain about how data and information can be preserved throughout the process of an operating system upgrade such as the on described in M3. I will go through the different types of backup as well as the procedure that should be followed to ensure that the information and data keeps its integrity so that it can be recovered if the installation or upgrade of the software did not work. 

The main way that this can be done is by backup up the information that is important on the hard drive so that it is not on their anymore and is instead in a n external storage medium like a USB stick or an external hard drive. At first you may think that you would have to backup the whole  hard drive in order to save all of the information that it contains, but this in fact would not work for a few reasons. To start off with one reason that this would not work 

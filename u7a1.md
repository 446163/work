...
### M1
With this issue the way that this could be fixed would be to have a different level of user access for all the different people that use the system. This would make it so that the staff only have access to the part of the system that they need and this would therefore mean that only people that are high up in the system or are trusted can edit the people who are on the plane and this would prevent things like this from happening. In addition to this the main way that this could also happen would be the hiring of staff that are not at the company with good intentions and these corrupt staff would abuse the system for their own good and this can be stopped by the hiring company. But the main reason here is the different levels of user access. For example, the staff that help manage the bookings 
. 
